...
def encode(image_to_hide, image_to_hide_in, n_bits):
    ...
    for y in range(height):
        for x in range(width):
            # (107, 3, 10)
            # most sig bits
            r_hide = hide_image[x, y]
            g_hide = hide_image[x, y]
            b_hide = hide_image[x, y]
            r_hide = get_n_most_significant_bits(r_hide[0], n_bits)
            g_hide = get_n_most_significant_bits(g_hide[1], n_bits)
            b_hide = get_n_most_significant_bits(b_hide[2], n_bits)

            # remove lest n sig bits
            r_hide_in = hide_in_image[x, y]
            # print ('Red Channel Value', r_hide)
            g_hide_in = hide_in_image[x, y]
            b_hide_in = hide_in_image[x, y]
            r_hide_in = remove_n_least_significant_bits(r_hide_in[0], n_bits)
            g_hide_in = remove_n_least_significant_bits(g_hide_in[1], n_bits)
            b_hide_in = remove_n_least_significant_bits(b_hide_in[2], n_bits)
...
if "__main__":
    image_to_hide_path = "./AirplaneForCrpt"
    image_to_hide_in_path = "./PeppersForCytp"
    encoded_image_path = "./encoded.tif"
    decoded_image_path = "./decoded.tif"
    n_bits = 2
